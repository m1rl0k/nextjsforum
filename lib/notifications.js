...
/**
 * Lightweight email sender (fail-soft). Requires SMTP_URL (e.g. smtp://user:pass@host:port)
 */
export async function sendEmailNotification({ to, subject, text }) {
  if (!to || !subject || !text) return;
  if (!process.env.SMTP_URL) {
    console.log('Email send skipped (no SMTP_URL)', { to, subject });
    return;
  }
  try {
    const transporter = nodemailer.createTransport(process.env.SMTP_URL);
    await transporter.sendMail({
      from: process.env.EMAIL_FROM || 'no-reply@forum.local',
      to,
      subject,
      text
    });
