...
export const loginSchema = z.object({
  email: z.string().email('Invalid email address').max(254, 'Email must be at most 254 characters').toLowerCase(),
...
// ============================================================================
// INSTALLATION WIZARD VALIDATION SCHEMAS
// ============================================================================
...
export const installDatabaseSchema = z.object({
  ...
  dbPort: z.string().regex(/^\d+$/, 'Port must be a number').optional(),
  dbName: z.string().max(64).optional(),
  dbUser: z.string().max(64).optional(),
  dbPassword: z.string().max(255).optional(),
}).refine((data) => {
  // For non-SQLite databases, require connection details
  if (data.dbType !== 'sqlite') {
    if (!data.dbHost || data.dbHost.trim() === '') return false;
    if (!data.dbName || data.dbName.trim() === '') return false;
  }
  return true;
}, {
  message: 'Database host and name are required for PostgreSQL and MySQL',
  path: ['dbHost']
});

// Admin account validation (stricter than regular registration)
...
export const installSiteConfigSchema = z.object({
  siteName: z.string()
    .min(2, 'Site name must be at least 2 characters')
    .max(100, 'Site name must be at most 100 characters')
    .regex(/^[^<>]*$/, 'Site name cannot contain < or >'),
  siteDescription: z.string()
    .max(500, 'Description must be at most 500 characters')
    .optional()
    .default(''),
  adminEmailContact: z.string()
...
export const installForumStructureSchema = z.object({
...
// Combined installation step validation
export const installStepSchema = z.object({
  step: z.number().int().min(1).max(5),
  data: z.object({}).passthrough(), // Allow any data, validate per step
});
